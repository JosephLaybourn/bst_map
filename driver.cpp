...
void test7() {
    ...
    for ( int i=0; i<10; ++i ) {
        ...
        std::uniform_int_distribution<unsigned int>      dis( 2, 12 );
        ...
        std::cout << "delete " << num_insert/2 << " elements\n";
        // delete some
        std::shuffle( map_content.begin(), map_content.end(), std::mt19937{std::random_device{}()} );
        std::copy( map_content.begin() + map_content.size() - num_insert/2, map_content.end(), std::ostream_iterator<int>(std::cout, " "));
        std::cout << "\n";
        simple_deletes( map, std::vector<int>( map_content.begin() + map_content.size() - num_insert/2, map_content.end() ) );
        data.insert( data.end(), map_content.begin() + map_content.size() - num_insert/2, map_content.end() );
        map_content.resize( map_content.size() - num_insert/2 );
...
void test10()
{
    ...
    data.resize( dis( gen ) );
    simple_inserts( map, std::vector<int>( data.begin(), data.end() ) );
...
void test12()
{
    ...
    simple_inserts( map2, std::vector<int>( data2.begin(), data2.end() ) );
